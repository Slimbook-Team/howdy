...
if out_value == config.get("core", "disabled-login", fallback=True):
	print(_("The disable-login option has already been set to ") + out_value)
	sys.exit(1)
...
for line in fileinput.input([config_path], inplace=1):
	print(line.replace("disabled-login = " + config.get("core", "disabled-login", fallback=True), "disabled-login = " + out_value), end="")

# Print what we just did
if out_value == "true":
	print(_("Howdy has been disabled at login"))
else:
	print(_("Howdy has been enabled at login"))
